...
class GoogleGIcon extends StatelessWidget {
  ...
  static const _redPath =
      'M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 '
...
class _GoogleGPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas
      ..scale(scale)
      ..drawPath(_parsePath(GoogleGIcon._bluePath), _fill(0xFF4285F4))
      ..drawPath(_parsePath(GoogleGIcon._greenPath), _fill(0xFF34A853))
      ..drawPath(_parsePath(GoogleGIcon._yellowPath), _fill(0xFFFBBC05))
      ..drawPath(_parsePath(GoogleGIcon._redPath), _fill(0xFFEA4335));
  }
...
Path _parsePath(String d) {
  ...
  (double, double) point(double x, double y, {required bool relative}) =>
  ...
  while (index < elements.length) {
    ...
    switch (code) {
      case 'M':
        final (x, y) = point(next(), next(), relative: relative);
        path.moveTo(x, y);
        cx = x;
        cy = y;
        subX = x;
      ...
      case 'V':
        ...
        cy = y;
      case 'C':
        final (x1, y1) = point(next(), next(), relative: relative);
        final (x2, y2) = point(next(), next(), relative: relative);
        final (x, y) = point(next(), next(), relative: relative);
...
bool _isCommand(String ch) => RegExp('[a-zA-Z]').hasMatch(ch);
