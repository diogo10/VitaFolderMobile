...
/// Host-runnable mirror of `integration_test/critical_path_test.dart`.
///
/// The `integration_test/` twin needs a device (`flutter drive`/emulator);
/// this mirror runs the same journey under plain `flutter_test` so
/// `flutter test --coverage` (and CI without an emulator) still pins the
/// onboarding → home → reminders → people contract. Keep the two in sync.
